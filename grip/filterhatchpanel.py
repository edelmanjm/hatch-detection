...
class GripPipeline:
    ...
    """
    __blur_radius = 2.7027027027027026
    blur_output = None

    __hsv_threshold_input = blur_output
    __hsv_threshold_hue = ntproperty("/SmartDashboard/hsv_threshold_hue", [0.0, 180.0], persistent=True)
    __hsv_threshold_saturation = ntproperty("/SmartDashboard/hsv_threshold_saturation", [0.0, 29.872665534804735],
                                                 persistent=True)
    __hsv_threshold_value = ntproperty("/SmartDashboard/hsv_threshold_value", [238.4892086330935, 255.0],
                                            persistent=True)
    ...
    __filter_contours_min_area = 1000.0
    __filter_contours_min_perimeter = 0.0
    __filter_contours_min_width = 0.0
    __filter_contours_max_width = 10000.0
    __filter_contours_min_height = 0.0
    __filter_contours_max_height = 10000.0
    __filter_contours_solidity = [0.0, 100]
    __filter_contours_max_vertices = 1.0E8
    __filter_contours_min_vertices = 0.0
    __filter_contours_min_ratio = 0.5
    __filter_contours_max_ratio = 1.5
    ...
    def __filter_contours(input_contours, min_area, min_perimeter, min_width, max_width,
        ...
        for contour in input_contours:
            ...
            val = math.fabs(1.0 - ((math.pi * (cv2.arcLength(contour, True) / (2.0 * math.pi)) ** 2) / area))
            if val > 0.3:
                continue
            ...
            # print(str(i) + "\t" + "\t" + str(cv2.arcLength(contour, True)) + "\t"+ str(area) + "\t" + str(val))
            # i += 1
        return output
